...
const addCurriculum = async (req, res) => {
  try {
    const curriculumExisted = await curriculumRepository.getCurriculumByCode(
      req.body.curriculum_code
    );
    if (curriculumExisted) {
      res.status(400).json({
        message: "Curriculum's code is already existed.",
      });
    } else {
...
const deleteCurriculumById = async (req, res) => {
  try {
    const curriculum = await curriculumRepository.deleteCurriculumById(
      req.params.id
    );
    res.status(201).json({
...
const searchCurriculums = async (req, res) => {
  try {
    ...
    const curriculums = await curriculumRepository.searchCurriculums(
      searchString,
      page,
      size
    );
    const total = await curriculumRepository.totalCurriculums(
      searchString,
      page,
      size
    );
